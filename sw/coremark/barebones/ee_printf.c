...
static int
ee_str_contains(const char *haystack, const char *needle)
{
...
#ifdef SIM_VERILATOR
    if (ee_str_contains(buf, "Errors detected"))
        platform_signal_fail(1);
    if (ee_str_contains(buf, "Cannot validate operation"))
        platform_signal_fail(2);
#endif
