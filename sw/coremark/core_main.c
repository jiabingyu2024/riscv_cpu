...
#if !defined(SIM_VERILATOR)
    /**
     * @details EEMBC公布的`list/matrix/state_known_crc`对应固定迭代与参数组合。
     * RTL仿真常缩短`ITERATIONS`，CRC必然不一致，会触发`Errors detected`与`SIM_DONE`失败。
     */
    if (known_id >= 0)
    {
        for (i = 0; i < default_num_contexts; i++)
...
#if !defined(SIM_VERILATOR)
    /**
     * @details 官方要求计时≥10s；仿真中`iterate()`周期远短于此，会误伤`total_errors`。
     */
