...
module ExecuteMemStage(
    ...
    always_ff @(posedge self.clk or posedge self.rst) begin
        if (self.rst) begin
            ...
            if (loadPipe1.valid) begin
                loadRsp <= loadPipe1;
                loadRsp.wb.data <= extend_load_data(loadPipe1.memSubType, dram.rdata);
            end
            loadPipe1 <= loadPipe0;
            loadPipe0 <= '0;
    ...
    always_comb begin
        ...
        ctrl.exStallReq = loadRsp.valid && memHasWork && !ctrl.exPipe.flush;
        ctrl.memStageEmpty = !loadRsp.valid && !loadPipe0.valid && !loadPipe1.valid;

        if (loadRsp.valid) begin
            self.nextMemToStage[0] = loadRsp.wb;
        end
            ...
            if (pipeReg[i].valid && !ctrl.exPipe.flush && !ctrl.exPipe.stall) begin
                if (is_store(pipeReg[i].subType.memSubType)) begin
                    ...
                    if (storeBuffer.StoreBufferMatchOut.hit) begin
                        self.nextMemToStage[loadRsp.valid ? 1 : 0].valid = 1'b1;
                        self.nextMemToStage[loadRsp.valid ? 1 : 0].Rd = loadLaunch.wb.Rd;
                        self.nextMemToStage[loadRsp.valid ? 1 : 0].writeRd = loadLaunch.wb.writeRd;
                        self.nextMemToStage[loadRsp.valid ? 1 : 0].robIndex = loadLaunch.wb.robIndex;
                        self.nextMemToStage[loadRsp.valid ? 1 : 0].data =
